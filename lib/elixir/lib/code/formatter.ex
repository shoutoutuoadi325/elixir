...
defmodule Code.Formatter do
  ...
  defp pop_comments(quoted) do
    pop_trailing? =
      case quoted do
        {_, meta, _} when is_list(meta) -> meta[:closing] == nil and meta[:end] == nil
        _ -> true
      end

    {leading, trailing, next_eol_count, quoted} =
      pop_comments(quoted, true, pop_trailing?)
  ...
  defp pop_comments({left, right}, pop_leading?, pop_trailing?) do
    ...
    {leading ++ right_leading, trailing ++ right_trailing,
     max(left_eol_count, right_eol_count), {left, right}}
  end
  ...
  defp pop_comments(list, pop_leading?, pop_trailing?) when is_list(list) do
    {list, {leading, trailing, next_eol_count}} =
      Enum.map_reduce(list, {[], [], 1}, fn quoted, {leading, trailing, next_eol_count} ->
        ...
        {quoted,
         {leading ++ new_leading, trailing ++ new_trailing,
          max(next_eol_count, new_eol_count)}}
      end)
