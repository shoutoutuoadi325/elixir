...
defmodule Code do
  ...
  **Warning**: Calling this function inside a macro is considered bad
  ...
  defp attach_comments_to_quoted({_, meta, _} = quoted, comments, boundary, collect?)
    ...
    boundary = max_comments_end_line(comments_end_line(form, meta), boundary)
    collect_children? = collect? or (form == :<<>> and meta[:delimiter] != nil)

    {args, comments, args_info} =
      attach_comments_to_quoted(args, comments, boundary, collect_children?)
  ...
  defp attach_leading_comments({form, meta, args}, comments) when is_list(meta) do
    case comments_position(meta) do
      nil ->
  ...
  defp max_comments_end_line(nil, boundary), do: boundary
  defp max_comments_end_line(line, nil), do: line
  defp max_comments_end_line(line, boundary), do: max(line, boundary)

  defp comments_position(meta) do
    if line = meta[:line], do: {line, meta[:column] || 0}
  end

  defp comments_end_line(:., _meta), do: nil

  defp comments_end_line(_form, meta) do
